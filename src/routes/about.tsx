...
function About(){
  ...
  return <>
    ...
    <Section size="none" className="pb-20 lg:pb-28"><Container className="grid gap-14 lg:grid-cols-2 lg:items-center"><Reveal><Img image={portrait} className="aspect-[4/5]"/></Reveal><Reveal className="lg:px-10"><p className="eyebrow">{copy(text,"story_eyebrow","My story")}</p><h2 className="mt-5 font-display text-3xl">{copy(text,"story_title","The camera taught me to notice.")}</h2><RichText html={copy(text,"story_body","")} className="mt-7 space-y-5 leading-8 text-muted-foreground"/><div className="mt-10"><p className="eyebrow">Elsewhere</p><SocialRail settings={settings} className="mt-5"/></div></Reveal></Container></Section>
