...
const schema = z.object({
  ...
  kind: z.string().min(2, "Tell me what kind of session you need."),
...
export const Route = createFileRoute("/contact")({
  ...
  loader: ({ context: { queryClient } }) =>
    Promise.all([
      ...
      queryClient.ensureQueryData(pageCopyQuery("contact")),
    ]),
  head: () => ({
    meta: [
      { title: "Contact — OnySnow Studios" },
      {
        name: "description",
        content:
          "Contact OnySnow Studios about photography sessions, events, and creative commissions.",
      },
      { property: "og:title", content: "Contact — OnySnow Studios" },
      {
        property: "og:description",
        content: "Start a conversation about your next photography project.",
      },
...
function Contact() {
  ...
  const form = useForm<Values>({
    ...
    defaultValues: { name: "", email: "", kind: "", message: "" },
  });
  ...
  return (
    <>
      <PageIntro
        loading={isPending}
        eyebrow={copy(text, "intro_eyebrow", "Contact")}
        title={copy(text, "intro_title", "Tell me what you’re imagining.")}
        body={copy(text, "intro_body", "")}
      />
      <section className="px-5 pb-28 sm:px-8 lg:px-12">
        <div className="mx-auto grid max-w-screen-xl gap-14 lg:grid-cols-[.7fr_1.3fr]">
            ...
            {settings?.["contact_email"] ? (
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <a className="mt-2 block" href={`mailto:${settings["contact_email"]}`}>
                    {settings["contact_email"]}
                  </a>
                </div>
              </div>
            ...
            {settings?.["service_area"] ? (
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="mt-2">{settings["service_area"]}</p>
                </div>
              </div>
            ) : null}
                ...
                render={({ field }) => (
                  <FormItem className="sm:col-span-2">
                    ...
                    <FormControl>
                      <Input
                        className="h-12"
                        placeholder="Event, portrait, creative shoot…"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
                ...
                render={({ field }) => (
                  <FormItem className="sm:col-span-2">
                    ...
                    <FormControl>
                      <Textarea
                        className="min-h-40"
                        placeholder="The mood, date, location, and anything else that matters"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
