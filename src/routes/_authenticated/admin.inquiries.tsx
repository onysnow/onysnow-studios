...
function InquiriesPage() {
  ...
  const rows = (inquiries.data ?? []).filter((i) =>
                ...
                >
                  ...
                  <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                    <Button
                      variant="ghost"
                      size="icon"
                      aria-label={i.status === "new" ? "Mark as read" : "Mark as unread"}
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      aria-label={i.status === "archived" ? "Restore" : "Archive"}
      ...
      <Dialog open={Boolean(open)} onOpenChange={(v) => !v && setOpen(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {open?.email} · {open?.kind}
            </DialogDescription>
          </DialogHeader>
          <p className="whitespace-pre-wrap text-sm leading-relaxed">{open?.message}</p>
          {open ? (
            <Button asChild variant="cinematic" className="mt-2 w-fit">
