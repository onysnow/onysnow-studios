  ...
  return (
    <>
      ...
      <Tabs defaultValue="json">
        <TabsList><TabsTrigger value="json">Record editor</TabsTrigger><TabsTrigger value="css">Custom styling</TabsTrigger></TabsList>
        <TabsContent value="json" className="mt-6"><JsonEditor /></TabsContent>
        <TabsContent value="css" className="mt-6"><CssEditor /></TabsContent>
      </Tabs>
    </>
  );
}
...
function JsonEditor() {
  ...
  const parsed = useMemo(() => {
    ...
    try {
      ...
      if (current && String(obj[pk]) !== String(current[pk])) {
        return { ok: false as const, error: `“${pk}” cannot be changed here — it identifies the record.` };
      }
      const unknownKeys = current ? Object.keys(obj).filter((k) => !(k in current)) : [];
      if (unknownKeys.length) {
        return { ok: false as const, error: `Unexpected field${unknownKeys.length === 1 ? "" : "s"}: ${unknownKeys.join(", ")}` };
      }
      const missing = current ? Object.keys(current).filter((k) => !(k in obj)) : [];
      if (missing.length) {
        return { ok: false as const, error: `Missing field${missing.length === 1 ? "" : "s"}: ${missing.join(", ")}` };
      }
      return { ok: true as const, value: obj };
    } catch (error) {
      return { ok: false as const, error: error instanceof Error ? error.message : "That is not valid JSON." };
    }
  }, [draft, current, pk]);
  ...
  async function commit() {
    ...
    try {
      ...
      toast.error("The database refused that change", { description: error instanceof Error ? error.message : undefined });
    }
  }

  function labelFor(row: Row) {
    return String(row["name"] ?? row["title"] ?? row["key"] ?? row["section_key"] ?? row["author"] ?? row[pk]);
  }
    ...
    <div className="max-w-4xl space-y-5">
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <Select value={table} onValueChange={(v) => setTable(v as EditableTable)}>
            ...
            <SelectContent>{EDITABLE_TABLES.map((t) => <SelectItem key={t} value={t}>{t.replace(/_/g, " ")}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <div className="space-y-2">
          <Label>Record</Label>
          <Select value={rowId} onValueChange={setRowId}>
            ...
            <SelectContent>
              {rows.map((r) => <SelectItem key={String(r[pk])} value={String(r[pk])}>{labelFor(r)}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
      </div>
      ...
      {current ? (
        <>
          ...
          {parsed.ok ? (
            <p className="flex items-center gap-2 text-sm text-muted-foreground"><Check className="size-4 text-primary" /> Valid JSON, all fields accounted for.</p>
          ) : (
          ...
          <div className="flex gap-3">
            <Button variant="cinematic" disabled={!parsed.ok || !changed} onClick={() => setConfirmOpen(true)}><Save /> Review and save</Button>
            <Button variant="ghost" disabled={!changed} onClick={() => setDraft(saved)}>Discard changes</Button>
          </div>
        </>
      ) : (
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <DialogFooter>
            ...
            <Button variant="cinematic" onClick={commit}>Save record</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function CssEditor() {
  ...
  async function save() {
    ...
    try {
      ...
      toast.error("Could not save the styling", { description: error instanceof Error ? error.message : undefined });
    }
  }
  ...
  return (
    <div className="max-w-4xl space-y-5">
      <p className="text-sm text-muted-foreground">
        These rules are added to every page of the website. They load last, so they win over the built-in styling.
        Leave it empty to go back to the original look.
      </p>
      ...
      {problem ? (
        ...
        <p className="flex items-center gap-2 text-sm text-muted-foreground"><Check className="size-4 text-primary" /> Brackets balanced — safe to apply.</p>
      )}
      <div className="flex gap-3">
        <Button variant="cinematic" disabled={Boolean(problem) || draft === null || value === saved} onClick={save}><Save /> Apply styling</Button>
        <Button variant="ghost" disabled={draft === null} onClick={() => setDraft(null)}>Discard changes</Button>
      </div>
    </div>
  );
}
...
function validateCss(source: string): string | null {
  ...
  if (/<\/?script/i.test(source) || /javascript:/i.test(source)) return "Scripts are not allowed in styling.";
  let depth = 0;
  ...
  if (depth !== 0) return `${depth} rule${depth === 1 ? "" : "s"} left unclosed — check your braces.`;
  return null;
}
