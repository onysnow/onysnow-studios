...
export function brokeredPreviewStorage() {
  ...
  const request = (
    ...
    new Promise((resolve) => {
      ...
      let done = false;
      // Declared before `finish` because that closure clears it, and assigned
      // after, because the timeout's callback calls `finish`. prefer-const sees
      // a single assignment and misses that `const` can't be declared empty.
